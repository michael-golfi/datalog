...
export const BUILTIN_PREDICATE_DOCS: ReadonlyMap<string, BuiltinPredicateDoc> = new Map([
  [
    'DefPred',
    {
      ...
      detail:
        'Use `DefPred` to make graph edges self-describing. It records the predicate id plus subject/object cardinality and storage types.',
  ...
  [
    'Edge',
    {
      ...
      detail:
        'Use `Edge(subject, predicate, object)` for the canonical graph layer. Pair it with `DefPred` and `meta/domain_class` / `meta/range_class` edges so graph algorithms can inspect schema as data.',
  ...
  [
    'TypeAndCardinality',
    {
      ...
      detail:
        'This is commonly used with `Smeta` and `Ometa` to bridge compound metadata into rule logic.',
