...
import { buildNodeSummaryTargets, buildGraphPredicateIds, buildGraphNodeIds, buildPredicateDefinitions, buildWorkspacePredicateIdentities } from './datalog-workspace-index-builders.js';
import { buildCompoundFieldNames, buildCompoundFieldSchemas, buildCompoundSchemaTargets, buildPredicateSchemaTargets } from './datalog-workspace-schema-index-builders.js';
...
import type { DatalogWorkspaceCompoundSchemaTarget, DatalogWorkspacePredicateSchemaTarget } from './datalog-workspace-schema-targets.js';
...
export interface DatalogWorkspaceIndexState {
  ...
  readonly predicateDefinitionsByIdentity: Map<string, readonly DatalogWorkspacePredicateDefinition[]>;
  ...
  readonly predicateSchemaTargetsById: Map<string, readonly DatalogWorkspacePredicateSchemaTarget[]>;
  readonly compoundSchemaTargetsByName: Map<string, readonly DatalogWorkspaceCompoundSchemaTarget[]>;
...
export function buildDatalogWorkspaceIndexState(options: {
  ...
  const indexedDocumentsByUri = new Map(options.documents.map((document) => [document.uri, document] as const));
