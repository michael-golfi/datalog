...
export function translateDatalogFactQuery(operation: SelectFactsOperation | DatalogQueryStatement): TranslatedSqlQuery {
  const selectFactsOperation = operation.kind === 'query'
    ? createSelectFactsOperationFromDatalogQuery(operation)
    : operation;
  const logicalPlan = compileSelectFactsLogicalPlan(selectFactsOperation, DEFAULT_GRAPH_PREDICATE_CATALOG);
