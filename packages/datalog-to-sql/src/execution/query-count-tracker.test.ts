...
const createPostgresSqlClient = vi.fn((connectionString: string) => {
  ...
  return Object.assign(sqlQuery, { unsafe: sqlUnsafe, end: sqlEnd }) as unknown as PostgresSqlClient;
});
...
describe('createQueryCountTracker', () => {
  ...
  it('counts each unsafe call while delegating to the wrapped client', async () => {
    ...
    await expect(tracker.sql.unsafe('select * from vertices where id = $1;', ['vertex/alice'])).resolves.toEqual([
      { id: 'vertex/alice' },
    ]);
    expect(tracker.getQueryCount()).toBe(1);
    expect(sqlUnsafe).toHaveBeenCalledWith('select * from vertices where id = $1;', ['vertex/alice']);
  });
  ...
  it('preserves the wrapped client template tag and end methods', async () => {
    ...
    await expect(tracker.sql`select count(*)::text as count from vertices`).resolves.toEqual([{ count: '1' }]);
