...
module.exports.run = async function run() {
  ...
  assert.equal(
    document.languageId,
    'datalog',
    'Smoke fixture did not resolve to the datalog language.',
  );
  ...
  if (expectFailure) {
    throw new Error(
      'DATALOG_SMOKE_BROKEN_EXPECTED_FAILURE: extension activation unexpectedly succeeded.',
    );
  }
  ...
  const completionLabels = await waitForCompletionLabels(
    document.uri,
    positionAfterSnippet(document, '  Sha'),
  );
  ...
  assert(
    symbolNames.includes('Parent/2'),
    'Smoke document symbols did not include grouped Parent/2.',
  );
  ...
  assert(
    hoverContents.includes('**Shared/2**'),
    `Smoke hover did not describe Shared/2. Received: ${hoverContents}`,
  );
  assert(
    hoverContents.includes('Arity: `2`'),
    `Smoke hover did not include Shared arity details. Received: ${hoverContents}`,
  );

  assert(
    definitions.some((definition) => isDefinitionAtLine(definition, smokeFilePath, 5)),
    'Smoke definition did not resolve Shared to the local definition line.',
  );
  assert(
    crossFileDefinitions.some((definition) => isDefinitionAtLine(definition, schemaFilePath, 1)),
    'Smoke definition did not resolve SchemaShared to the sibling workspace file.',
  );

  assert(
    completionLabels.includes('Shared'),
    'Smoke completions did not include Shared for the incomplete clause.',
  );

  assert(
    foldingRanges.some(
      (range) => getFoldingRangeStartLine(range) === 0 && getFoldingRangeEndLine(range) === 1,
    ),
    `Smoke folding ranges did not include the leading comment block. Received: ${JSON.stringify(
      foldingRanges,
    )}`,
  );
  assert(
    foldingRanges.some(
      (range) => getFoldingRangeStartLine(range) === 8 && getFoldingRangeEndLine(range) === 11,
    ),
    `Smoke folding ranges did not include the multiline UsesShared clause. Received: ${JSON.stringify(
      foldingRanges,
    )}`,
  );

  assert(
    semanticLegend.tokenTypes.includes('comment'),
    'Smoke semantic token legend did not include comment.',
  );
  assert(
    semanticLegend.tokenTypes.includes('function'),
    'Smoke semantic token legend did not include function.',
  );
  assert(
    semanticLegend.tokenTypes.includes('property'),
    'Smoke semantic token legend did not include property.',
  );
  assert(
    semanticLegend.tokenModifiers.includes('definition'),
    'Smoke semantic token legend did not include the definition modifier.',
  );
  assert(
    decodedSemanticTokens.some((token) => token.tokenType === 'comment' && token.line === 0),
    'Smoke semantic tokens did not include the leading comment token.',
  );
  assert(
    ...
    'Smoke semantic tokens did not include the Shared definition token.',
  );
  assert(
    decodedSemanticTokens.some((token) => token.tokenType === 'property' && token.line === 10),
    'Smoke semantic tokens did not include the graph predicate property token.',
  );
};
...
async function waitForDocumentSymbols(documentUri) {
  return waitForProviderResult({
    ...
    request: () =>
      vscode.commands.executeCommand('vscode.executeDocumentSymbolProvider', documentUri),
...
async function waitForHoverContents(documentUri, position) {
  const hovers = await waitForProviderResult({
    ...
    request: () =>
      vscode.commands.executeCommand('vscode.executeHoverProvider', documentUri, position),
...
async function waitForDefinitions(documentUri, position) {
  return waitForProviderResult({
    ...
    request: () =>
      vscode.commands.executeCommand('vscode.executeDefinitionProvider', documentUri, position),
    isReady: (result) => Array.isArray(result) && result.length > 0,
  });
}
...
async function waitForCompletionLabels(documentUri, position) {
  const completionList = await waitForProviderResult({
    ...
    request: () =>
      vscode.commands.executeCommand('vscode.executeCompletionItemProvider', documentUri, position),
...
async function waitForFoldingRanges(documentUri) {
  return waitForProviderResult({
    ...
    request: () =>
      vscode.commands.executeCommand('vscode.executeFoldingRangeProvider', documentUri),
    isReady: (result) => Array.isArray(result) && result.length > 0,
  });
}
...
async function waitForSemanticTokensLegend(documentUri) {
  return waitForProviderResult({
    ...
    request: () =>
      vscode.commands.executeCommand('vscode.provideDocumentSemanticTokensLegend', documentUri),
    isReady: (result) =>
      result && Array.isArray(result.tokenTypes) && Array.isArray(result.tokenModifiers),
  });
}
...
async function waitForSemanticTokens(documentUri) {
  return waitForProviderResult({
    ...
    request: () =>
      vscode.commands.executeCommand('vscode.provideDocumentSemanticTokens', documentUri),
    isReady: (result) =>
      result &&
      Array.isArray(Array.from(result.data ?? [])) &&
      Array.from(result.data ?? []).length > 0,
  });
}
...
function collectDocumentSymbolNames(symbols) {
  return symbols.flatMap((symbol) => [
    symbol.name,
    ...collectDocumentSymbolNames(symbol.children ?? []),
  ]);
}
    ...
    if (seen === occurrenceIndex) {
      return {
        range: new vscode.Range(
          ...
          document.positionAt(offset + snippet.length),
        ),
      };
    }
...
function isDefinitionAtLine(definition, expectedFilePath, expectedLine) {
  ...
  const targetRange =
    'targetSelectionRange' in definition ? definition.targetSelectionRange : definition.range;
